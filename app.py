...
def models(matrix):
    ...
    complete_df = pd.read_csv(matrix)
    ...
    if request.method == 'POST':     
        ...
        for i, d in enumerate(distances[0]):
          ...
          top = 5
          itm_.extend(complete_df[complete_df.userId == indices[0][i]].nlargest(top,'ratings').itemId.values)
...
def predict():
    ...
    dic = {'0': 'Bearded Goat' , '1':'Western Rise' , '2': 'KOTN', '3': 'johnnie-O' , '4': 'XTRATUF' , "5": 'Cotopaxi', '6': "Roark", '7': 'Taylor Stitch',     # Branch 1
	       ...
	       '32': 'Lululemon', '33': 'RVCA', '34': 'Vineyard Vines', '35': 'Burberry', '36': 'Hollister', '37': 'Pendelton', '38': 'Life is Good', '39': 'Bridge & Burn'}      # Branch 4
            
    names = ['RM_NN_Ufeats_Ifeats.csv', 'RM_normalized.csv']
    
    for n in names:
        new_pred = []
        itm_ = models(n)
        for i in range(len(itm_)):
            if str(itm_[i]) in dic.keys():
                new_pred.append(dic[str(itm_[i])])
            else:
                new_pred.append(itm_[i])
            
        new_pred_dic[n] = new_pred
    
    zip_list = zip(['NN model with user and item features', 'NN model with user feature only'], new_pred_dic.values())
